...
import type { ChildDashboard, SubjectMastery, WeeklyDigest } from '@/lib/parent/digest';

export type { ChildDashboard, SubjectMastery, WeeklyDigest };
...
export async function getDashboardData(): Promise<ChildDashboard[]> {
  ...
  // Skills catalog (for per-subject totals).
  const { data: skills } = await supabase.from('skills').select('id, subject_code');
  const skillRows = (skills ?? []) as Array<{ id: string; subject_code: string }>;
  ...
  const { data: masteryRows } = await supabase
    .from('skill_mastery')
    .select('child_id, skill_id, status, current_level')
    .in('child_id', childIds);
  const masteryByChild = new Map<string, Array<{ skill_id: string; status: string; current_level: number }>>();
  for (const m of (masteryRows ?? []) as Array<{ child_id: string; skill_id: string; status: string; current_level: number }>) {
    const list = masteryByChild.get(m.child_id) ?? [];
    list.push(m);
    masteryByChild.set(m.child_id, list);
  }

  // Learning events in the last 30 days (sessions + milestones).
