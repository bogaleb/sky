...
export default function LetterLab({ childId, nickname = 'friend', onExit }: LetterLabProps) {
  ...
  const advance = useCallback(
      ...
      if (i + 1 >= next.length) {
        ...
        void (async () => {
          try {
            ...
            await bumpQuestProgress(childId, 'writing_game', 1);
            await awardStickers(childId, ['pen-pal']);
            await checkTrophies(childId, 'writing_done').catch(() => {});
            await logLearningEvent(childId, 'milestone', {
              metadata: { kind: 'letter_lab_win', letters: next.length, retries },
            });
          } catch {
    ...
    [retries, nickname, childId, beginLetter]
  );
      ...
      {phase === 'won' && (
        <div className="animate-kid-pop-in mx-4 flex w-full max-w-md flex-col items-center rounded-kid-card bg-white/95 px-8 py-8 text-center shadow-2xl">
          ...
          <h2 className="mt-3 text-3xl font-black text-kid-ink-900 md:text-4xl">
            Great writing, {nickname}!
          </h2>
          <p className="mt-2 text-lg font-bold text-kid-ink-700">
            You traced all {SESSION_LENGTH} letters and earned
          </p>
